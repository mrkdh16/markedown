...
>1. The variance of the gradient at $x^{(i)}$ is $\mathcal{C}^{fnn}(i)=1$. 
>2. The covariance is $\mathcal{C}^{fnn}(i,j)=\frac{1}{2^L}$.
...
>1. the variance is $\mathcal{C}^{res}_{\beta,BN}(i) = \beta^2(L-1)+1$;
>2. the covariance is $\mathcal{C}^{res}_{\beta,BN}(i,j) \sim \beta\sqrt{L}$; and
...
>Proof Strategy.
>The covariance defines an [[Inner Products|inner product]] on the [[Vector Spaces|vector space]] of real-valued random variables with mean zero and finite second moment. It was shown in Balduzzi et al. (2015); Balduzzi (2016) that the gradients in neural nets are sums of path-weights over active paths, see section A3. The first step is to observe that path-weights are orthogonal with respect to the variance inner product. To express gradients as linear combinations of path-weights is thus to express them over an orthogonal basis. 
>
