...
The first tool borrowed from probability is the binomial random variable. Let $X \sim \text{Binomial}(n,x)$ where $x$ is some arbitrary real number in $[0,1]$. Recall that $P(X=k)=\binom{n}{k}x^k(1-x)^{n-k}$ , $\mathbb{E}\left( \frac{X}{n} \right)=x$, $\text{Var}\left( \frac{X}{n} \right) = \frac{1}{n^2}\text{Var}(X) = \frac{nx(1-x)}{n^2} = \frac{x(1-x)}{n}$, and that Chebyshev's Inequality states that 
$$
...
$$
Chebyshev's Inequality is the other main engine of the proof.
...
$$
where we used the fact that $\sum^n_{k=0}P(X=k)=1$. Then,
$$
\left |\sum^n_{k=0}P(X=k)f(\frac{k}{n})-f(x)\sum^n_{k=0}P(X=k)\right| = \sum^n_{k=0}P(X=k)\left|f(\frac{k}{n})-f(x)\right|.
$$
In order to use our two inequalities (one from uniform continuity and the other from Chebyshev), we split the sum into two sums. We do this by splitting the indices $\{0,1,\dots,n\}$ into two sets based on some parameter $\eta>0$:
$$
...
We have successfully bounded both terms we had set out to bound. And we have obtained our desired bound:
$$
